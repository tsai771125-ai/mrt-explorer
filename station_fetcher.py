"""
station_fetcher.py
用 google-genai (新版 SDK) + Gemini 2.0 Flash 抓取捷運站點特色，
...
def fetch_station_data(station_id: str, station_name: str) -> dict:
    """主要函式：Gemini 2.0 Flash 生成站點資料 + Wikipedia 照片"""
    base = {
    ...
    if not photo_url:
        photo_url, credit = get_wikipedia_photo(f"{station_name} Taipei Taiwan")
    base["photo_url"] = photo_url
    base["photo_credit"] = credit

    # ── 3. 每個 highlight 補獨立照片 ───────────────
